...
foreach ( $items as $item_id => $item ) :
	...
	if (!$item["composite_parent"]){  //it does not have a composite parent so it must be a normal product (or the composite parent)
	?>
		<tr class="compositeRow"><td style="text-align:left; vertical-align:middle; border: 1px solid #eee; word-wrap:break-word;">
	<?php }else{ ?>
		<tr><td td colspan="3"  style="text-align:right; vertical-align:middle; border: 1px solid #eee; word-wrap:break-word;">
	<?php }
			...
			if(isset($item['item_meta']['pa_ounces'])){

				echo '<br/><small>' . nl2br( 'Ounces: ' . ucfirst($item['item_meta']['pa_ounces'][0]) ) . '</small>';
			}

			if(isset($item['item_meta']['pa_cups'])){

				echo '<br/><small>' . nl2br( 'Cups: ' . ucfirst($item['item_meta']['pa_cups'][0]) ) . '</small>';
			}
			// SKU
...
//			}
			...
			// Variation
	$prod_size = get_post_meta($item['variation_id'])['attribute_pa_size'][0];
			if($prod_size !== null){
				echo '<br/><small>' . nl2br( 'Size: ' . ucfirst($prod_size) ) . '</small>';
			}
	// just use our version because theirs doesn't always work
//			if ( $item_meta->meta ) {
...
//			}
		...
		?></td>
		<?php 
		if (!$item["composite_parent"]){  //it doesn't ahve a compostite parent, so put the other two columns in ?>		
